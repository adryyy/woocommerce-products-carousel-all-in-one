    ...
    public function generate($atts) {
        ...
        */
        ...
        while ( $loop->have_posts() ) {
            ...
            $this->product_url = esc_url( get_permalink(apply_filters('woocommerce_in_cart_product', $this->product->get_id())) );
            $this->shop_url = esc_url( get_permalink(wc_get_page_id('shop')));


            $this->image = wp_get_attachment_image_src( get_post_thumbnail_id($this->product->get_id()), $this->params['image_source'] );
            ...
            if ( $this->params['show_featured_image'] === 'true' ) {
                ...
                $featured_image = '<div class="woocommerce-products-carousel-all-in-one-image">';
                    $featured_image .= '<a href="' . $this->product_url . '" title="' . __('Show item', 'woocommerce-products-carousel-all-in-one') . ' ' . $this->product->get_name() . '">';
                        $featured_image .= '<img alt="' . $this->product->get_name() . '" style="max-width:' . $this->params['image_width'] . '%;max-height:' . $this->params['image_height'] . '%" ' . $data_src . $image_class . '>';
                    $featured_image .= '</a>';
                $featured_image .= '</div>';
            ...
            if ( $this->params['show_title'] === 'true' ) {
                $title = '<h3 class="woocommerce-products-carousel-all-in-one-title">';
                    $title .= '<a href="' . $this->product_url . '" title="' . $this->product->get_name() . '">' . $this->product->get_name() . '</a>';
                $title .= '</h3>';
            ...
            if ( $this->product->is_on_sale() ) {
				$percentage = round( ( (  $this->product->get_regular_price()  ) / $this->product->get_sale_price() ) * 10 );
                $this->price .= '<span class="woocommerce-products-carousel-all-in-one-sale">' . __('On sale', 'woocommerce-products-carousel-all-in-one').': ' . $percentage. ' !</span>';
            }

            if ( $this->params['show_price'] == 'true' ) {
                $this->price_value = $this->product->get_price_html();
            ...
            if ( $this->params['show_add_to_cart_button'] === 'true' || $this->params['show_more_button'] === 'true' || $this->params['show_more_items_button'] === 'true' ) {
                ...
                if ( $this->params['show_add_to_cart_button'] === 'true' ) {
                    if ( $this->product->is_in_stock() ) {
                        $buttons .= sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="woocommerce-products-carousel-all-in-one-add-to-cart-button button %s get_type()_%s" title="%s">%s</a>',
                            ...
                            esc_attr( $this->product->get_type() ),
                ...
                if ( $this->params['show_more_button'] === 'true' ) {
                    if( $this->product->is_downloadable() ) {
                        $buttons .= '<a href="' . $this->product_url . '" class="woocommerce-products-carousel-all-in-one-more-button button" title="' . __('Download', 'woocommerce-products-carousel-all-in-one') . ' ' . $this->product->get_name() . '">' . __('<i class="icon-download-alt icon-"></i> download', 'woocommerce-products-carousel-all-in-one') . '</a>';
                    } else {
                        $buttons .= '<a href="' . $this->product_url . '" class="woocommerce-products-carousel-all-in-one-more-button button" title="' . __('Show item', 'woocommerce-products-carousel-all-in-one') . ' ' . $this->product->get_name() . '">' . __('show item', 'woocommerce-products-carousel-all-in-one') . '</a>';
                    }
                }
    ...
    private function carousel($params = array()) {
        ...
        $out = '<script type="text/javascript">
                    $(document).ready(function(e) {
                        var wooCommerceCarousel' . $params['id'] . ' = $("#woocommerce-products-carousel-all-in-one-' . $params['id'] . '");
                        $("#woocommerce-products-carousel-all-in-one-' . $params['id'] . '").owlCarousel({
